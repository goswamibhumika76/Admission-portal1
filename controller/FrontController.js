...
//cloudinary setup
cloudinary.config({
  cloud_name: "dom7gdjxr",
  api_key: "148185262283176",
  api_secret: "mLhy27vkvGjjQ5GySyOIeO2y_Q8",
});
...
class FrontController {
  ...
  static studentInsert = async (req, res) => {
    try {
      ...
      if (data) {
        const token = jwt.sign({ ID: data._id }, "Bhumika@123");
        // console.log(token)
    ...
    try {
      ...
      if (user != null) {
        ...
        if (isMatched) {
          // if (user.role == 'admin') {
          //   //token create
          //   const token = jwt.sign({ ID: user._id }, "Bhumika@123");
          //   // console.log(token)
          //   res.cookie("token", token);
  ...
  static profile = async (req, res) => {
    try {
      const { name, image, email, id } = req.userdata;
      res.render("profile", { n: name, e: email, i: image, msg: req.flash('success') })
    } catch (error) {
      console.log(error);
    }
  };
